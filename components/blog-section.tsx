...
export function BlogSection() {
  ...
  useEffect(() => {
    ...
    if (savedPosts) {
      ...
      // Patch the broken placeholder image if it exists in cache
      let modified = false
      parsed = parsed.map((post: BlogPost) => {
        if (post.id === "1" && post.image.includes("placeholder.svg")) {
          modified = true
          return { ...post, image: "/Rio_azul_flutuando.webp" }
        }
        return post
      })
      
      if (modified) {
        localStorage.setItem("blogPosts", JSON.stringify(parsed))
      }
  ...
  useEffect(() => {
    if (posts.length > 0) {
      gsap.fromTo(".blog-section-card-gsap", 
        { y: 30, opacity: 0 }, 
        { 
          y: 0, 
          opacity: 1, 
  ...
  return (
    <section id="blog" className="py-16 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {posts.slice(0, 8).map((post) => (
            <div key={post.id} className="blog-section-card-gsap opacity-0">
              <BlogCard post={post} />
            </div>
          ))}
        </div>
