...
import type { Attraction as SupabaseAttraction } from "@/services/supabase-attractions"
...
interface AttractionsSectionProps {
  initialAttractions?: Attraction[] | null
}

export function AttractionsSection({ initialAttractions }: AttractionsSectionProps = {}) {
  const { t } = useLanguage()
  ...
  useEffect(() => {
    if (initialAttractions != null && initialAttractions.length > 0) return
    const loadAttractions = async () => {
      try {
        const res = await fetch("/api/attractions")
        if (!res.ok) throw new Error("Failed to fetch attractions")
        const attractionsData: SupabaseAttraction[] = await res.json()
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredAttractions.map((attraction, index) => (
            <Card
              key={attraction.id}
              className="group overflow-hidden transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 hover:shadow-green-100/60 border border-gray-100 bg-white rounded-2xl"
            ...
            >
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute top-4 left-4">
                  <Badge className="bg-white/90 text-gray-800 backdrop-blur-sm shadow-md transition-transform duration-300 group-hover:scale-105">{getCategoryLabel(attraction.category)}</Badge>
                </div>
                <div className="absolute top-4 right-4 bg-white/90 rounded-full px-2 py-1 flex items-center gap-1 backdrop-blur-sm shadow-md">
              ...
              <CardContent className="p-6">
                <h3 className="text-xl font-bold mb-2 group-hover:text-green-700 transition-colors duration-300">{attraction.title}</h3>
        ...
        {filteredAttractions.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-600 mb-2">{t("noAttractionsTitle")}</h3>
            <p className="text-gray-500">{t("noAttractionsDesc")}</p>
          </div>
        )}
      </div>
    </section>
