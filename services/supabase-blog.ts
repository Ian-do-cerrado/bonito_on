...
export async function getAllBlogPosts(client?: any): Promise<BlogPost[]> {
  ...
  try {
    ...
    const [tagsResult, keywordsResult, galleryResult] = await Promise.all([
      db.from("blog_post_tags").select("post_id, tag").in("post_id", postIds),
      db.from("blog_post_seo_keywords").select("post_id, keyword").in("post_id", postIds),
      db
    ...
    galleryResult.data?.forEach((image: any) => {
...
export async function createBlogPost(post: Omit<BlogPost, "id">): Promise<BlogPost | null> {
  try {
    ...
    return {
      ...
      excerpt_es: blogData.excerpt_es,
