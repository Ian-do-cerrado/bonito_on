...
import { getTranslatedBlogTitle, getTranslatedBlogExcerpt, getTranslatedBlogContent } from "@/lib/dynamic-translations"
...
export default function BlogPostPage() {
  ...
  const nextGalleryImage = () => {
    if (post?.gallery && post.gallery.length > 0) {
      ...
      setCurrentGalleryIndex((prev) => (prev + 1) % length)
    }
  }
  ...
  const prevGalleryImage = () => {
    if (post?.gallery && post.gallery.length > 0) {
      const length = post.gallery.length
      setCurrentGalleryIndex((prev) => (prev - 1 + length) % length)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative h-96 pt-16">
        <Image src={post.image || "/placeholder.svg"} alt={getTranslatedBlogTitle(post, post.title, language)} fill className="object-cover" />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <article className="bg-white rounded-lg shadow-sm p-8">
          <header className="mb-8">
            <h1 className="text-4xl font-bold text-gray-900 mb-6">{getTranslatedBlogTitle(post, post.title, language)}</h1>
          ...
          <div className="prose prose-lg max-w-none">
            <p className="text-xl text-gray-700 mb-8 font-medium">{getTranslatedBlogExcerpt(post, post.excerpt, language)}</p>

            <div className="text-gray-700 leading-relaxed space-y-6">
              <div dangerouslySetInnerHTML={{ __html: getTranslatedBlogContent(post, post.content, language) || t("contentUnavailable") }} />
