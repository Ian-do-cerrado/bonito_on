...
export async function submitContactForm(data: SubmitContactFormData) {
  try {
    ...
    const safeCheckin = data.checkin ? escapeHtml(data.checkin) : ""
    const safeGuests = data.guests ? escapeHtml(data.guests) : ""
    const safeAttraction = data.attraction ? escapeHtml(data.attraction) : ""
    ...
    const phoneLink = `tel:+55${formattedWhatsApp}`
              ...
              <div style="background: linear-gradient(135deg, #f0fdf4 0%, #ecfdf5 100%); border: 2px solid #16a34a; border-radius: 12px; padding: 25px; margin-bottom: 30px; position: relative;">
                ...
                <div style="margin-top: 10px;">
                  <div style="margin-bottom: 20px; padding: 15px; background: white; border-radius: 8px; border-left: 4px solid #16a34a;">
                    ...
                    <span style="color: #1f2937; font-size: 18px; font-weight: 600;">${safeName}</span>
                  </div>
                  ...
                  <div style="margin-bottom: 20px; padding: 15px; background: white; border-radius: 8px; border-left: 4px solid #8b5cf6;">
                    ...
                    <a href="mailto:${safeEmail}" style="color: #1f2937; font-size: 16px; font-weight: 600;">${safeEmail}</a>
                  </div>

                  ${safeCheckin ? `
                  <div style="margin-bottom: 20px; padding: 15px; background: white; border-radius: 8px; border-left: 4px solid #f97316;">
                    <strong style="color: #374151; display: block; margin-bottom: 5px; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">📅 Data prevista</strong>
                    <span style="color: #1f2937; font-size: 16px;">${safeCheckin}</span>
                  </div>
                  ` : ""}

                  ${safeGuests ? `
                  <div style="margin-bottom: 20px; padding: 15px; background: white; border-radius: 8px; border-left: 4px solid #ec4899;">
                    <strong style="color: #374151; display: block; margin-bottom: 5px; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">👥 Viajantes</strong>
                    <span style="color: #1f2937; font-size: 16px;">${safeGuests}</span>
                  </div>
                  ` : ""}

                  ${safeAttraction ? `
                  <div style="margin-bottom: 20px; padding: 15px; background: white; border-radius: 8px; border-left: 4px solid #0ea5e9;">
                    <strong style="color: #374151; display: block; margin-bottom: 5px; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">🎯 Interesse</strong>
                    <span style="color: #1f2937; font-size: 16px;">${safeAttraction}</span>
                  </div>
                  ` : ""}
                  
                  <div style="margin-bottom: 20px; padding: 15px; background: white; border-radius: 8px; border-left: 4px solid #3b82f6;">
                    <strong style="color: #374151; display: block; margin-bottom: 5px; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">🕐 Data/Hora do envio</strong>
                    <span style="color: #1f2937; font-size: 16px;">${timestamp}</span>
                  </div>

                  ${safeMessage ? `
                  <div style="padding: 15px; background: white; border-radius: 8px; border-left: 4px solid #f59e0b;">
                    <strong style="color: #374151; display: block; margin-bottom: 5px; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">💬 Mensagem</strong>
                    <span style="color: #1f2937; font-size: 16px;">${safeMessage}</span>
                  </div>
                  ` : ""}
                </div>
              </div>
...
E-mail: ${data.email}
${data.checkin ? `Data prevista: ${data.checkin}` : ""}
${data.guests ? `Viajantes: ${data.guests}` : ""}
${data.attraction ? `Interesse: ${data.attraction}` : ""}
Data/Hora do envio: ${timestamp}
...
Sistema de notificações v2.0
    ...
    console.error("❌ Erro ao enviar email de lead:", {
      ...
      contact: data.name,
      destination: getLeadDestinationEmail(),
      timestamp: new Date().toISOString(),
    })
    ...
    if (error instanceof Error) {
      if (
        ...
        error.message.includes("Configuração de email")
      ) {
        return {
          success: false,
          error: "Serviço de email temporariamente indisponível. Entre em contato pelo WhatsApp.",
        }
      }
      ...
      if (
        ...
        return {
          success: false,
          error: "Não foi possível enviar sua mensagem agora. Entre em contato pelo WhatsApp.",
        }
      }
