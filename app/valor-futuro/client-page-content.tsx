...
export function ClientValorFuturoPage({ initialTours }: ClientValorFuturoPageProps) {
  ...
  const [activeCategory, setActiveCategory] = useState<TourData["category"]>("all")
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-6 sm:mb-8">
          ...
          <Tabs
            ...
            onValueChange={(value) => setActiveCategory(value as TourData["category"])}
            ...
            <div className="relative overflow-hidden">
              <TabsList ref={tabsRef} className="flex overflow-x-auto scrollbar-hide p-1 bg-gray-100 rounded-md w-full">
                {["all", "resort", "floating", "adventure", "waterfall", "contemplation", "biking", "pantanal", "scubaDiving", "rappelling", "cave"].map((cat) => (
                  ...
                  >
                    {getCategoryTitle(cat as TourData["category"])}
                  </TabsTrigger>
                ))}
              </TabsList>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          {filteredTours.map((tour, index) => (
            ...
            >
              <TourCard tour={tour} basePath="/valor-futuro" />
            </div>
          ))}
        </div>
      </div>
